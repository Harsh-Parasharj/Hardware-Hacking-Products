...
void displayInfoScreen() {
  display.clearDisplay();
  u8g2_for_adafruit_gfx.setFont(u8g2_font_unifont_t_symbols);

  // === ANIMATED HOODIE DRAW ===
  // Head outline
  for (int r = 0; r <= 12; r++) {
    display.clearDisplay();
    display.drawCircle(64, 30, r, WHITE);
    display.display();
    delay(20);
  }

  // Hoodie bottom line animation
  for (int x = 52; x <= 76; x++) {
    display.drawPixel(x, 42, WHITE);
    display.display();
    delay(10);
  }

  // Hoodie sides (animated)
  for (int i = 0; i <= 24; i++) {
    display.drawLine(52, 42, 52 + i / 2, 42 - i, WHITE); // Left
    display.drawLine(76, 42, 76 - i / 2, 42 - i, WHITE); // Right
    display.display();
    delay(25);
  }
  ...
  for (int b = 0; b < 3; b++) {
    ...
    display.drawPixel(68, 28, WHITE);
    display.display();
    delay(180);
    display.fillRect(59, 27, 10, 3, BLACK); // "blink"
    display.display();
    delay(120);
  }
  ...
  // === INFO SCREEN WITH FADE-IN TEXT ===
  display.clearDisplay();
  u8g2_for_adafruit_gfx.setFont(u8g2_font_baby_tf);

  const char* lines[] = {
    "CREATED BY HACKERS FOR HACKERS",
    "For More Product And Info",
    "Mail: hack897055@gmail.com"
  };
  int yPos[] = {20, 34, 48};

  for (int i = 0; i < 3; i++) {
    for (int flick = 0; flick < 2; flick++) { // flicker animation
      display.clearDisplay();
      u8g2_for_adafruit_gfx.setCursor(7, yPos[i]);
